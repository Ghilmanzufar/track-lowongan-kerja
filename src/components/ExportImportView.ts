...
export function renderExportImportView(container: HTMLElement): void {
  ...
  container.querySelector('#btnLoadDemo')?.addEventListener('click', async () => {
    if (confirm('Muat data simulasi demo? Seluruh data yang ada saat ini akan digantikan dengan data demo.')) {
      await loadSeedData();
  ...
  container.querySelector('#btnResetAll')?.addEventListener('click', async () => {
    if (confirm('PERINGATAN: Apakah Anda yakin ingin menghapus SELURUH data lamaran dan tugas? Tindakan ini tidak dapat dibatalkan.')) {
